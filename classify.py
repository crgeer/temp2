...
def main():
    ...
    with tf.Session() as sess:
        ...
        with tf.variable_scope("", reuse=tf.AUTO_REUSE):
            logits, layers = get_logits(new_input, lengths)
        ...
        print(layers.keys())
        data = sess.run(layers["layer_2"], {new_input: [audio], lengths: [length]})
        print(tf.math.reduce_sum(data))
        print(sess.run(tf.math.reduce_sum(data), {new_input: [audio], lengths: [length]}))
        print(data.shape)
